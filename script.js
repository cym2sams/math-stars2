...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function main() {
        ...
        // 使用 localStorage 進行離線數據儲存
        const CONSTANTS = { STORAGE_KEY: 'multiplicationStarChartLeaderboard_v1', classes: { '2A': 25, '2B': 25, '2C': 25, '2D': 25, '2E': 25, '老師': ["陳子殷老師", "陳綺雯老師", "陳懿文老師", "楊靖霖老師", "陳慧淇老師"] } };
        ...
        const functions = {
            ...
            endGame() { 
                ...
                ELEMENTS.timerDisplay.classList.remove('timer-warning'); 
                if (state.score > 0) functions.saveToLeaderboard(state.currentUser.name, state.score); 
                
                const rank = functions.getPlayerRank(state.currentUser.name); 
                let rankMessage = ""; 
                if (rank > 0 && rank <= 10) { 
                ...
                functions.displayLeaderboard(ELEMENTS.leaderboardContainer, state.currentUser.name); 
                functions.showScreen('end'); 
            },
            getPlayerRank(playerName) { 
                const board = functions.getFullLeaderboard(); 
            ...
            saveToLeaderboard(playerName, playerScore) { 
                let board = functions.getFullLeaderboard(); 
                const i = board.findIndex(p => p.name === playerName); 
                if (i > -1) { 
                    if (playerScore > board[i].score) { board[i].score = playerScore; } 
                } else { 
                    board.push({ name: playerName, score: playerScore }); 
                } 
                localStorage.setItem(CONSTANTS.STORAGE_KEY, JSON.stringify(board)); 
            },
            displayLeaderboard(container, currentPlayerName = null) { 
                container.innerHTML = '<h2>排行榜</h2>'; 
                const board = functions.getLeaderboard(); 
                if (board.length === 0) { 
            ...
            showVisualAid() { 
                ...
                const r = Math.min(state.currentQuestionNums.n1, state.currentQuestionNums.n2), c = Math.max(state.currentQuestionNums.n1, state.currentQuestionNums.n2); 
                for (let i = 0; i < r; i++) { 
